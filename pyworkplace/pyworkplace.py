...
class Page(object):
    ...
    def handle_webhook(
        self, payload, optin=None, message=None,
        echo=None, delivery=None, postback=None,
        read=None, account_linking=None, referral=None,
    ):
        ...
        for event in get_events(data):
            if event.is_optin:
                ...
                print(
                    'Webhook received unknown messagingEvent: {}'.format(
                        event,
                    ),
                )
    ...
    def get_postback_callbacks(self, event):
        ...
        for key in self._button_callbacks.keys():
            ...
            if self._button_callbacks_key_regex[key].match(
